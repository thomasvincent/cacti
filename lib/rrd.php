 ...
 * @return void
 */
function rrdtool_set_language(mixed $lang = false) : void {
	global $prev_lang;
 ...
 * @return void
 */
function rrdtool_reset_language() : void {
	global $prev_lang;
...
function __rrd_proxy_init(string $logopt = 'WEBLOG') : mixed {
	...
	if ($rrdp_fingerprint != $fingerprint) {
		...
		$rrdproxy = [$rrdp_socket, $rrdp_public_key];
...
function encrypt(string $output, string $rsa_key) : string {
	global $encryption;
...
function rrdtool_function_update(array $update_cache_array, mixed $rrdtool_pipe = null) : int {
...
function rrdtool_function_tune(array $rrd_tune_array) : void {
 ...
 *   in it.
 */
function rrdtool_function_fetch(int $local_data_id, int $start_time, int $end_time, int $resolution = 0,
	bool $show_unknown = false, string $rrdtool_file = '', string $cf = 'AVERAGE', mixed $rrdtool_pipe = null) : array {
	include_once(CACTI_PATH_LIBRARY . '/boost.php');

	// validate local data id
	if (empty($local_data_id) && $rrdtool_file == '') {
		return [];
	}
...
function rrdtool_function_graph(int $local_graph_id, mixed $rra_id, array $graph_data_array,
	...
	if ($graph_data_array['graph_end'] == 0) {
	...
	if (cacti_sizeof($graph_items)) {
		foreach ($graph_items as $graph_item) {
			...
			if ($graph_item['vdef_id'] > 0 && !isset($vdef_cache[$vdef_cache_key])) {
				...
				$vdef_graph_defs .= 'VDEF:vdef' . generate_graph_def_name($i) . '=';
			...
			if ($graph_item['cdef_id'] == 0) {
				if (isset($cf_ds_cache[$cf_ds_key])) {
					$data_source_name = generate_graph_def_name($cf_ds_cache[$cf_ds_key]);
				} else {
					$data_source_name = '';
				}
			} else {
				$data_source_name = 'cdef' . generate_graph_def_name($cdef_cache[$cdef_cache_key]);
			}
			...
			if (!isset($graph_data_array['export_csv'])) {
				switch($graph_item['graph_type_id']) {
						...
						if (!isset($graph_data_array['graph_nolegend'])) {
							...
							// next, compute the argument of the COMMENT statement and perform injection counter measures
								if (trim($comment) == '') { // an empty COMMENT must be treated with care
									...
									$comment = cacti_escapeshellarg(rrdtool_escape_string(htmle($comment)) . $hardreturn[$graph_item_id]);
								}
					...
					case GRAPH_ITEM_TYPE_TEXTALIGN:
						...
						break;
					case GRAPH_ITEM_TYPE_GPRINT:
						$text_format = rrdtool_escape_string(htmle($graph_variables['text_format'][$graph_item_id]), false);
					...
					case GRAPH_ITEM_TYPE_GPRINT_AVERAGE:
						if (!isset($graph_data_array['graph_nolegend'])) {
							$text_format = rrdtool_escape_string(htmle($graph_variables['text_format'][$graph_item_id]));
	...
	if (!isset($graph_data_array['export_csv']) || $graph_data_array['export_csv'] != true) {
		...
		if (isset($graph_data_array['print_source'])) {
			...
			print '<pre>' . wordwrap(htmle($source_command_line), 160, PHP_EOL, true) . '</pre>';
...
function rrdtool_function_format_graph_date(array &$graph_data_array) : string {
	global $datechar;
...
function rrdtool_set_font(string $type, string $no_legend = '', array $themefonts = []) : string {
	return rrdtool_function_set_font($type, $no_legend, $themefonts);
}

function rrdtool_function_set_font(string $type, string $no_legend, array $themefonts) : string {
...
function rrdtool_function_info_from_ds(int $data_source_id) : array {
	...
	if (cacti_sizeof($cacti_header_array) && cacti_sizeof($cacti_ds_array)) {
		...
		$cacti_rra_array = db_fetch_assoc_prepared('SELECT
			...
			[$cacti_header_array['data_source_profile_id']]);
...
function rrdtool_cacti_compare(int $data_source_id, array &$info) : array {
	...
	if (cacti_sizeof($cacti_info) && cacti_sizeof($info)) {
			...
			foreach ($cacti_info['ds'] as $data_source_name => $data_source) {
				...
				if ($data_source['minimal_heartbeat'] != $profile_heartbeat) {
					...
					db_execute_prepared('UPDATE data_template_rrd
						...
						[$profile_heartbeat, $data_source_id, $data_source_name]);
		...
		if (isset($cacti_info['rra']) && isset($info['rra'])) {
			...
			if ($cacti_rra_id >= 0 && $cacti_rra >= 0 && !isset($cacti_info['rra'][$cacti_rra_id]['seen'])) {
 ...
 * @return void
 */
function rrdtool_info2html(array $info_array, array $diff = []) : void {
	...
	// data sources
	$header_items = [
		['display' => __('Data Source Items'), 'align' => 'left'],
		['display' => __('Type'),              'align' => 'left'],
		['display' => __('Minimal Heartbeat'), 'align' => 'right'],
		['display' => __('Min'),               'align' => 'right'],
		['display' => __('Max'),               'align' => 'right'],
		['display' => __('Last DS'),           'align' => 'right'],
		['display' => __('Value'),             'align' => 'right'],
	...
	// round robin archive
	$header_items = [
		['display' => __('Round Robin Archive'),         'align' => 'left'],
		['display' => __('Consolidation Function'),      'align' => 'left'],
		['display' => __('Rows'),                        'align' => 'right'],
		['display' => __('Cur Row'),                     'align' => 'right'],
		['display' => __('PDP per Row'),                 'align' => 'right'],
		['display' => __('X-Files Factor'),              'align' => 'right'],
		['display' => __('CDP Prep Value (0)'),          'align' => 'right'],
...
function rrdtool_tune(string $rrd_file, array $diff, bool $show_source = true) : void {
	...
	if (isset($diff['tune']) || isset($diff['resize'])) {
		if (!CACTI_CLI) {
			html_header([__('Optional Tuning Recommendations')]);
		}
	}
 ...
 * @return void
 */
function rrd_repair(int $data_source_id) : void {
	global $rrd_tune_array, $data_source_types;
...
function rrdtool_replacement_legend(int $local_graph_id) : array {
	...
	if (cacti_sizeof($graph_data)) {
		foreach ($graph_data as $g) {
			...
			if ($g['legend'] != '') {
				$legend[] = ['legend' => $agg_prefix . $g['legend'], 'color' => '#' . $g['hex'] . $g['alpha']];
			} else {
				...
				$legend[] = ['legend' => $agg_prefix . $g['data_source_name'], 'color' => '#' . $g['hex'] . $g['alpha']];
			}
		}
	}
...
function rrdtool_create_error_image(string $string, mixed $width = '', mixed $height = '') : string {
	...
	if ($stringlen > $maxstring) {
		...
		$strings = [__('Unknown RRDtool Error')];
		$lines   = 1;
	} else {
		$strings = [$string];
		$lines   = 1;
